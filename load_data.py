...
def make_data(data,time_step,batch_size,dimension,missing_rate=0.2):
	...
	X = np.swapaxes(X,0,1) # 交换前两个维度，使得矩阵的形状变为 (batch_size,time_step,dimension) 
...
def get_batch(np_data,time_step,batch_size,dimension,missing_rate):
	...
	"""
	num = np_data.shape[0]
	X,Y = make_data(np_data,time_step,int(num/time_step),dimension,missing_rate)
	dataloader = make_dataloader(X,Y,batch_size)
	return dataloader
